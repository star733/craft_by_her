...
powershell -Command "'{0:N2} MB' -f ((Get-ChildItem .git -Recurse -Force | Measure-Object -Property Length -Sum).Sum / 1MB)"
echo.
...
git add PERFORMANCE_OPTIMIZATION_GUIDE.md QUICK_FIX_STEPS.md
...
git rm -r --cached server/uploads/*.jpg 2>nul
git rm -r --cached server/uploads/*.jpeg 2>nul
git rm -r --cached server/uploads/*.png 2>nul
git rm -r --cached node_modules 2>nul
git rm -r --cached client/node_modules 2>nul
git rm -r --cached server/node_modules 2>nul
git rm -r --cached realtime-tracking/node_modules 2>nul
git rm -r --cached selenium-tests/venv 2>nul
echo.
