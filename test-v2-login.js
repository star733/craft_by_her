// Test hub manager v2 login
const http = require('http');

async function testV2Login() {
  try {
    console.log('🧪 Testing hub manager v2 login...');
    ...
    const options = {
      hostname: 'localhost',
      port: 5000,
      path: '/api/hub-managers-v2/login',
      method: 'POST',
      headers: {
      ...
      res.on('end', () => {
        try {
          ...
          if (res.statusCode === 200 && response.success) {
            console.log('✅ Hub manager v2 login is working!');
    ...
    console.error('❌ Error testing v2 login:', error.message);
  }
}
